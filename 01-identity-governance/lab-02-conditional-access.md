...
2. **Block legacy authentication** — Eliminates a major attack vector (legacy auth bypasses MFA)
3. **Location-based policy** — Requires MFA from untrusted locations
...
3. In the left menu, click **Named locations**
...
2. **Name the policy:** `CA-01: Require MFA for Administrators`
...
4. Click **Target resources** (formerly Cloud apps):
   - Select **All cloud apps**

5. Click **Conditions** → **Sign-in risk** — Leave at default (Not configured)
...
7. Under **Enable policy**, select **Report-only**
8. Click **Create**
...
2. Click **Insights and reporting** (or check the **Sign-in logs**)
3. View which sign-ins would have been affected
...
6. **Grant**: **Block access** → Click **Select**

7. **Enable policy**: **Report-only** (test first)
...
1. In Conditional Access, click **+ New policy**
2. **Name:** `CA-03: Require MFA from Untrusted Locations`

### Step 4.2 — Configure Assignments

3. **Users:** `grp-sc500-contributors` (target the developer group)

4. **Target resources:** Select specific apps → **Microsoft Azure Management**
...
3. Click **What If**
4. ✅ Review which policies would apply and what controls would be enforced
...
2. Click each policy → Edit → **Enable policy:** Off → Save
3. Then delete the policy: Edit → Delete
