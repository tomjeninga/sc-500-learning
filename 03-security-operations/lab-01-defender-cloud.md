...
1. Search for **Microsoft Defender for Cloud** in the top search bar
1. Click the result — this opens the Defender for Cloud overview
...
1. Note your initial Secure Score — you'll improve it during this lab

### Step 1.3 — Review Environment Settings

1. In the left menu, click **Environment settings**
1. Expand your subscription
1. ✅ You should see the subscription with Defender plans status
1. **Foundational CSPM** should already be enabled (it's always free)
...
1. Find **Servers** in the Defender plans list
1. Toggle **Servers** to **On**
1. Select **Plan 1** (lower cost for lab purposes)
1. Click **Save**

### Step 2.2 — Verify plan is enabled

1. Return to Defender for Cloud overview
1. ✅ The servers plan should show as active
...
1. Scroll down to see **Security controls** list
1. Note the controls with the highest **Potential score increase**
1. ✅ Common high-impact controls:
...
1. Click on any recommendation (e.g., "Enable MFA for accounts with owner permissions")
1. Review:
...
1. Click on the recommendation
1. Review affected storage accounts
1. Click **Quick fix** (if available) OR follow the remediation steps:
   - Click the storage account name
   - Go to **Configuration**
   - Under **Secure transfer required**, select **Enabled**
   - Click **Save**

1. Return to the recommendation — the resource should now show as **Healthy**
...
1. Click on the **Not Configured** tab
1. Find your test VM (`vm-frontend-01`)
1. Click **Enable JIT on 1 VM**
1. Review the default rules (RDP port 3389, SSH port 22, WinRM 5985/5986)
1. Click **Save**

### Step 5.2 — Request JIT access

1. On the **Configured** tab, select your VM
...
1. Set:
   ...
   - **Time range:** 3 hours
1. Click **Open ports**
1. ✅ The NSG is temporarily updated to allow your IP on the requested port
