...
1. Configure storage/database service to use the Key Vault key
1. Rotate keys on schedule (automatic rotation supported)
1. If the key is disabled/deleted → data becomes inaccessible
...
1. An NSG has these inbound rules: Priority 100 Allow port 443 from Internet; Priority 200 Deny all from Internet. What happens to an HTTP (port 80) request from the internet?

1. Your WAF is in Detection mode and you're seeing SQLi attempts in the logs. What change do you make, and what impact should you test for?

1. A developer wants to SSH to a VM using its private IP. The VM has no public IP. What Azure service enables this?

1. What is the minimum subnet size for `AzureBastionSubnet`?

1. A storage account needs to be accessible only from a specific VNet subnet. What two options exist, and which provides better security?

1. An organization must prove that the keys encrypting their Azure SQL database are controlled exclusively by them and cannot be accessed by Microsoft. What technology and key storage option do you recommend?
